...
const JobOrderModal = props => {
	...
	useEffect(() => {
		if (!hasId) {
			...
			getItemDetails(data.item_id)
				.then(res => {
					const { orderedItems, itemDeliveryDetails, jobOrderDetails, inventory, joSeries } = res;
					setDetails({
						orderedItems,
						itemDeliveryDetails,
						jobOrderDetails,
	...
	const { orderedItems, itemDeliveryDetails, jobOrderDetails, inventory, joSeries } = itemDetails;
